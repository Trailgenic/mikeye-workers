import libraryData from "../datasets/ma-library.json" with { type: "json" };
import ontologyData from "../datasets/ontology.json" with { type: "json" };
import {
  BUILD,
  BUYER_LENS_AUDIT,
  DECISION_FRAMES,
...
export function originDataset() {
  return {
    ...
    name: ENTITY.name,
    role: ENTITY.role,
    domains: [SITE_ORIGIN, ...AFFILIATED_ENTITIES.map((entity) => entity.domain)],
...
export function exitDiagnostic() {
  const scores = [
    { id: "revenue_quality", name: "Revenue Quality", report_lenses: ["revenue_quality"] },
    { id: "founder_dependence", name: "Founder Dependence", report_lenses: ["founder_dependence"] },
    { id: "financial_health", name: "Financial Health", report_lenses: ["revenue_quality", "diligence_pressure"] },
    { id: "competitive_position", name: "Competitive Position", report_lenses: ["buyer_psychology", "ai_exposure"] },
    { id: "timing_alignment", name: "Timing Alignment", report_lenses: ["timing"] }
  ];
  ...
  return {
    ...
    dimension_model: {
      summary: "Eight owner answers produce five indicative scores. The paid report uses a 26-question intake and six broader buyer lenses.",
      self_reported: scores,
      total_dimensions: 5,
      report_dimensions: BUYER_LENS_AUDIT.dimensions.map(({ id, name }) => ({ id, name })),
      total_report_dimensions: 6,
      mapping_basis: "Each free score supplies context for the listed report lenses. The report uses additional intake information and framework analysis; the five scores and six lenses are separate structures. Competitive Position supplies context for Buyer Psychology and AI Exposure, rather than separately scoring either lens."
    },
    note: "The interactive diagnostic runs in the browser at execution_url. This MCP tool returns its structure and routing. AI generates the paid report from the submitted intake using Mike Ye's Buyer-Lens Audit framework.",
    ...stamp()
  };
}

const DATASET_BUILDERS = {
  ma_library: () => libraryData,
