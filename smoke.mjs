...
check('library contains 16 resources', library.resources.length === 16);
check('library contains ten verified published workbooks', library.resources.filter(r => r.download).length === 10);
...
check('synergy relates to working checklists and LOI economics', synergy.relatedResources.length === 4 && synergy.relatedResources.every(id => library.resources.some(r => r.id === id && r.relatedResources.includes(synergy.id))));
...
check('LOI guide has reciprocal working-resource relationships', loi?.relatedResources?.length === 4 && loi.relatedResources.every(id => library.resources.some(r => r.id === id && r.relatedResources?.includes(loi.id))));
